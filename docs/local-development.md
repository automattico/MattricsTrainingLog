...
`scripts/dev-router.php` includes `tests/stubs/mattwarden.php`, applies the default Mattwarden headers and CSP, serves allowlisted static types, rejects dotfiles, and dispatches flat endpoints with `.php` aliases and `PATH_INFO`.
...
node tests/js/ai-response-tests.js
```
