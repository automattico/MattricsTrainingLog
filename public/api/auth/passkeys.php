...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    mattrics_send_json(['passkeys' => $list]);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        ...
        // Require passkey re-authentication to confirm deletion
        if (empty($_SESSION['webauthn_challenge'])) {
            mattrics_send_json(['error' => 'No challenge in session. Request a delete challenge first.'], 400);
        }
        ...
        try {
            ...
            if ($matchIndex === null) {
                mattrics_send_json(['error' => 'Unknown credential used for authentication.'], 401);
            }

            $matched = $store['credentials'][$matchIndex];

            $webAuthn->processGet(
                mattrics_b64url_decode((string) ($body['assertion']['clientDataJSON'] ?? '')),
            ...
            mattrics_send_json(['error' => 'Authentication failed: ' . $e->getMessage()], 401);
        }
